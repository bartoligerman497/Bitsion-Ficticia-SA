...
(function (a) {
  ...
  d.ext.renderer.pageButton.bulma = function (f, k, D, E, n, v) {
    var w = new d.Api(f),
      p = f.oLanguage.oPaginate,
      F = f.oLanguage.oAria.paginate || {},
      h,
      l,
      x = 0,
      A = function (t, y) {
        var z,
          G = function (q) {
            q.preventDefault();
            a(q.currentTarget.firstChild).attr("disabled") ||
              w.page() == q.data.action ||
              w.page(q.data.action).draw("page");
          };
        var u = 0;
        for (z = y.length; u < z; u++) {
          var g = y[u];
          if (Array.isArray(g)) A(t, g);
          else {
            l = h = "";
            var r = "a";
            var m = !1;
              ...
              ((r = a("<li>", {
                ...
                .append(
                  a("<" + r + ">", {
                    ...
                    class: "pagination-link " + l,
                    disabled: m,
                  }).html(h)
                )
                .appendTo(t)),
              f.oApi._fnBindAction(r, { action: g }, G),
              x++);
          }
        }
      };
    try {
      var B = a(k).find(c.activeElement).data("dt-idx");
    } catch (t) {}
    var C = a(
  ...
  a(c).on("init.dt", function (f, k) {
    "dt" === f.namespace &&
      ((f = new a.fn.dataTable.Api(k)),
      a("div.dataTables_length select", f.table().container()).wrap(
        '<div class="select">'
      ));
  });
  return d;
